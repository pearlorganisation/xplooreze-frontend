...
export async function getFavorites() {
    try {
        const response = await api.get('/get-favorites', { params: { limit: 1000 } });

        if (response.data.success === true) {
            return {
                ...response.data.data,
                tutors: response.data.data.tutors.map(t => ({ ...t, isFavourite: true }))
            };
        }
...
export async function getBookings({ status }) {
    try {
        const response = await api.get('/get-bookings', { params: { statuses: [status], limit: 1000 } });
