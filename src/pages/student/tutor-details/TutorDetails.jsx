        ...
        {sevenDays.map((date) => {
          ...
          return (
            <div
              ...
              className={`calendar-day-column ${isCurrentDay ? "today" : ""}`}
            >
...
export default function TutorDetail() {
  ...
  const isProfileComplete = user && user.isFormSubmitted;
  ...
  const tutorMaxPrice =
    tutor?.max_hour_charge || (tutor?.min_hour_charge ? null : ""); // Handle null max price
  ...
  async function fetchTutors() {
    try {
      const { tutors, pagination } = await getTutors(
        new URLSearchParams({ similarTo: id, limit: 10 }),
      );
      console.log(tutors);
      setTutors(tutors);
    } catch (error) {
    ...
    try {
      ...
      if (!isTrial) {
        ...
        if (isNaN(price) || price < tutorMinPrice) {
          setSubmitError(
            `Price must be at least ${tutorMinPrice} ${tutorCurrency}.`,
          );
          setShowForm(false);
          return;
        }
        if (tutorMaxPrice !== null && price > tutorMaxPrice) {
          setSubmitError(
            `Price must be between ${tutorMinPrice} and ${tutorMaxPrice} ${tutorCurrency}.`,
          );
          setShowForm(false);
          return;
        }
      ...
      if (isSingleSession) {
        ...
        if (
          !bookingForm.dateRange ||
          !Array.isArray(bookingForm.dateRange) ||
          bookingForm.dateRange.length !== 2
        ) {
      ...
      if (!isTrial) {
        ...
        const startDateTime = new Date(
          `2000-01-01T${bookingForm.startTime}:00`,
        );
        ...
        if (endDateTime - startDateTime < 45 * 60 * 1000) {
          setSubmitError(
            "The end time must be at least 45 minutes after the start time.",
          );
          setShowForm(false);
          return;
        }
  ...
  const isSingleSession = isTrial || bookingForm.sessionType === "single";
  const dateLabel = isSingleSession
    ? "Schedule (Date) *"
    : "Schedule (Date Range) *";
  const selectsRangeProp = !isSingleSession;
  const placeholderText = isSingleSession
    ? "Select a single date (future only)"
    : "Select start and end dates (future only)";

  const hasOfflineModes =
    tutoringModes.length > 0 &&
    !tutoringModes.every((mode) => mode === "Online Tutoring");
    ...
    <>
      {isLoading ? null : (
        <div className="availability-section">
          ...
          {tutor &&
          tutor.avl_days &&
          tutor.avl_days.length > 0 &&
          tutor.avl_time &&
          tutor.avl_time.length > 0 ? (
              ...
              <div className="details-section">
                <div className="header">
                  <img
                    src={
                      tutor.profilePhoto &&
                      !tutor.profilePhoto.startsWith("http")
                  ...
                  <div className="vertical">
                    ...
                    <div className="info-tile">
                      <FaHourglass /> {tutor.class_duration || "None"}
                    </div>
                    <div className="info-tile">
                      <FaClock /> {tutor.teaching_experience || "None"} Years
                    </div>
                ...
                <h2>Specialty</h2>
                <div className="vertical">
                  <div className="info-tile detailed">
                    <b>Category: </b>
                    {tutor.category || "None"}
                  </div>
                  <div className="info-tile detailed">
                    <b>Teaching Level: </b>
                    {tutor.teaching_level || "None"}
                  </div>
                </div>
  ...
  const bookingTitle = isTrial
    ? `Book a Trial with ${tutor?.fullName}`
    : `Book ${tutor?.fullName}`;
            ...
            <div className="booking-section">
              {!isProfileComplete ? (
                <>
                  ...
                  >
                    <p>
                      Please complete your account details before you can book a
                      session.
                    </p>
                    <p style={{ marginTop: "10px" }}>
                      This helps us and the tutor provide the best experience
                      for you.
                    </p>
                    <button
                    ...
                    <form onSubmit={handleSubmit} className="booking-form">
                      ...
                      {!isTrial && (
                        <div className="form-group">
                          ...
                          <input
                            ...
                            min={tutorMinPrice}
                            max={
                              tutorMaxPrice !== null ? tutorMaxPrice : undefined
                            } // Only set max if it's not null
                            required
                          />
                        </div>
                      )}
                      ...
                      <div className="form-group">
                        ...
                        <DatePicker
                          ...
                          selected={
                            isSingleSession ? bookingForm.dateRange : undefined
                          }
                          startDate={
                            !isSingleSession
                      ...
                      {!isSingleSession && (
                        <div className="form-group">
                          ...
                          <MultiSelect
                            ...
                            onChange={(items) =>
                              handleMultiSelectChange("availableDays", items)
                            }
                          />
                        </div>
                      )}
