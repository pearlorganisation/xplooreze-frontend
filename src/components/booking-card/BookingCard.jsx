...
// Student-specific
import { createBookingOrder, verifyBookingPayment, submitRating } from '../../data/modules/student-module';
...
function getLocalDateStr(date, timeZone) {
    ...
    try {
        ...
        console.warn(`[getLocalDateStr] Invalid timezone "${timeZone}". Falling back.`);
...
function getTimezoneOffsetMs(timeZone) {
    ...
    try {
        ...
        console.warn(`[getTimezoneOffsetMs] Failed to compute offset for ${timeZone}: ${e.message}`);
        return 0;
    }
}
...
function calculateTotals(booking, isTutor) {
    ...
    try {
        ...
        if (isTutor) {
            ...
            return { displayAmount: totalAmountForStudent, totalSessions: totalSessions, breakdown: null };
        }
    } catch (err) {
...
export default function BookingCard({ booking, isTutor, onAccept, onReject }) {
    ...
    const [isEarlyJoinWindow, setIsEarlyJoinWindow] = useState(false);
    const [isClassEndedToday, setIsClassEndedToday] = useState(false);
    ...
    const earlyJoinTimerRef = useRef(null);
    // --- DATA EXTRACTION (CONDITIONAL) ---
    const student = booking.studentId || {};
    const tutor = booking.tutorId || booking.tutor || {};
    const displayUser = isTutor ? student : tutor;
    const displayName = displayUser.fullName || (isTutor ? 'Unknown Student' : 'Unknown Tutor');

    const profilePhoto = displayUser.profilePhoto && !displayUser.profilePhoto.startsWith('http')
    ...
    useEffect(() => {
        ...
        clearTimeout(earlyJoinTimerRef.current);
        if (booking.status === 'completed' || booking.status === 'rejected' || booking.status === 'expired') {
            setIsClassTime(false);
            setIsEarlyJoinWindow(false);
            setIsClassEndedToday(false);
            return;
        }
        const now = new Date();
        ...
        const todayDayAbbr = new Intl.DateTimeFormat('en-US', { timeZone: tutorTimezone, weekday: 'short' }).format(now);
        const startDateStr = getLocalDateStr(booking.startDate, tutorTimezone);
        const endDateStr = booking.endDate ? getLocalDateStr(booking.endDate, tutorTimezone) : null;
        let isClassDay = false;
        if (booking.sessionType === 'single') {
            isClassDay = (todayInTutorTzStr === startDateStr);
        } else {
            const isTodayInDateRange = (todayInTutorTzStr >= startDateStr) &&
                (endDateStr ? todayInTutorTzStr <= endDateStr : true);
            const isTodayADayOfWeek = booking.availableDays.includes(todayDayAbbr);
            isClassDay = isTodayInDateRange && isTodayADayOfWeek;
        }
        if (!isClassDay) {
            setIsClassTime(false);
            setIsEarlyJoinWindow(false);
        ...
        const classStartTimeUTC = constructLocalDateTime(todayInTutorTzStr, booking.startTime, tutorTimezone);
        const classEndTimeUTC = constructLocalDateTime(todayInTutorTzStr, booking.endTime, tutorTimezone);
        if (classStartTimeUTC && classEndTimeUTC) {
            ...
            const earlyStartMs = startMs - earlyJoinMs;

            // If today's class time has ended, mark it ended so UI doesn't show "Join at ..."
            setIsClassEndedToday(nowMs > endMs);

            if (nowMs < earlyStartMs) {
                setIsEarlyJoinWindow(false);
                earlyJoinTimerRef.current = setTimeout(() => {
                    setIsEarlyJoinWindow(true);
                }, earlyStartMs - nowMs);
            } else if (nowMs >= earlyStartMs && nowMs < startMs) {
                setIsEarlyJoinWindow(true);
                earlyJoinTimerRef.current = setTimeout(() => {
                    setIsEarlyJoinWindow(false);
                }, startMs - nowMs);
    ...
    const canTutorJoinEarly = isTutor && isOnline && isActive && isEarlyJoinWindow;
    const showScheduleInfo =
        isOnline && isActive && !canStartClass && !(isTutor && canTutorJoinEarly) && !isClassEndedToday;
    // --- HANDLERS (ALL) ---
    ...
    const handlePay = async () => {
        ...
        try {
            const orderResponse = await createBookingOrder({ bookingId: booking._id });
                ...
                onSuccess: async (response) => {
                    try {
                        ...
                        setDialogData({
                            ...
                            message: 'Your payment was successful and the booking is now active.',
                            confirmText: 'OK',
                            onConfirm: () => {
            ...
            setDialogData({
                isOpen: true,
                title: 'Payment Error',
                message: error.message || error || 'Unable to start payment. Try again later.',
                confirmText: 'Close',
                onConfirm: () => setDialogData(prev => ({ ...prev, isOpen: false })),
            });
        }
            ...
            <div className="breakdown-overlay" onClick={() => setShowBreakdown(false)}>
                <div className="breakdown-content" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="breakdown-item">
                        ...
                        <span>{paymentDetails.breakdown.durationHours.toFixed(1)} hours</span>
                    </div>
                    <div className="breakdown-item">
                        <span>Price per Hour:</span>
                        <span>{paymentDetails.breakdown.pricePerHour.toFixed(2)} {booking.currency}</span>
                    </div>
                    <div className="breakdown-item">
                        <span>Gross Amount:</span>
                        <span>{paymentDetails.breakdown.grossAmount.toFixed(2)} {booking.currency}</span>
                    </div>
                    <div className="breakdown-item">
                        <span>Commission (5%):</span>
                        <span>{paymentDetails.breakdown.commission.toFixed(2)} {booking.currency}</span>
                    </div>
                    <div className="breakdown-item">
                        <span>GST on Commission (18%):</span>
                        <span>{paymentDetails.breakdown.gstOnCommission.toFixed(2)} {booking.currency}</span>
                    </div>
                    <div className="breakdown-item total">
                        <span>Your Earning:</span>
                        <span>{paymentDetails.breakdown.tutorEarning.toFixed(2)} {booking.currency}</span>
                    </div>
            ...
            <div className="booking-header">
                <div className="tutor-info">
                    <img
                        ...
                        onError={(e) => { e.target.src = '/app-logo-black.png'; }}
                    />
                    <div className="tutor-details">
                        <h3 className="tutor-name">{displayName}</h3>
                        {!isTutor && (
                            <p className="tutor-headline">{tutor.headline || 'Experienced Tutor'}</p>
                        )}
                ...
                <div className="status-container">
                    ...
                    {booking.status === 'rejected' && (
                        <p className="rejection-note">Reason: {booking.rejectionReason || 'No reason provided'}</p>
                    )}
                </div>
            </div>
            {/* --- BODY (COMMON) --- */}
                ...
                <div className="detail-grid">
                    ...
                    <div className="detail-tile">
                        <div className="tile-content">
                            ...
                            <span className="tile-value">
                                {formatLocalDate(booking.startDate)}<br />
                    ...
                    <div className="detail-tile">
                        <div className="tile-content">
                            ...
                            <span className="tile-value">
                                {paymentDetails.totalSessions} Session{paymentDetails.totalSessions !== 1 ? 's' : ''}
                                <br />
                                {booking.isTrial ? (
                                    ...
                                    <>
                                        <span className="price-value">
                                            {paymentDetails.displayAmount.toFixed(2)} {booking.currency}
                    ...
                    {booking.sessionType === 'multiple' && (
                        <div className="detail-tile">
                            <div className="tile-content">
                                ...
                                <span className="tile-value">
                                    {booking.availableDays.join(', ')}<br />
                                    {booking.endDate ? `Ends ${formatLocalDate(booking.endDate)}` : 'Ongoing'}
                                </span>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="tile-content">
                            ...
                            <div className="mode-tags">
                                {booking.tutoringMode.map(mode => {
                                    const modeClass = mode === 'Online Tutoring' ? 'online' : 'in-person';
                                    const Icon = mode === 'Online Tutoring' ? FaVideo : FaMapMarkerAlt;
                                    return (
                            ...
                            <div className="rating-content">
                                {isTutor ? (
                                    ...
                                    <>
                                        ...
                                        />
                                        {booking.rating && (
                                            <span className="rating-label">You rated {booking.rating}/5</span>
                                        )}
                                    </>
                                )}
                            </div>
            ...
            <div className="booking-footer">
                ...
                <div className="price-section">
                    {booking.isTrial || !booking.price ? (
                        ...
                        <>
                            <span className="price-amount">{booking.price} {booking.currency}</span>
                ...
                <div className="action-section">
                    {isTutor ? (
                        ...
                        isPending ? (
                            ...
                            <>
                                {(canStartClass || canTutorJoinEarly) && (
                                    <button className="start-class-btn" onClick={handleStartClass}>
                                        {canTutorJoinEarly ? 'Join Room' : 'Start Class'}
                                    </button>
                                )}
                                {showScheduleInfo && (
                                    <div className="start-class-btn future-schedule" style={{ cursor: 'default' }}>
                                        <span className="join-text">Join Class at</span>{' '}
                                        <span className="date-time">
                                            {formatLocalDate(booking.startDate)} {formatTime(booking.startTime)}
                                        </span>
                                    </div>
                                )}
                                {isOnline && isActive && isClassEndedToday && (
                                    <div className="start-class-btn future-schedule" style={{ cursor: 'default' }}>
                                        <span className="join-text">Class ended</span>
                                    </div>
                                )}
                            </>
                        )
                        ...
                        <>
                            ...
                            {/* Show Pay Now button only if accepted AND it's not a trial */}
