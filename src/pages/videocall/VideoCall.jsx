...
const VideoCall = () => {
    ...
    if (!currentUser.cb_id || !opponentUser.cb_id || !currentUser.email) {
        ...
        return <div className="error-screen">Missing user credentials for video call.</div>;
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            const poll = () => {
                if (!isMountedRef.current) {
                    return reject(new Error('Component unmounted while waiting for file data.'));
                }
                ...
                if (fileData && fileData.dataURL) {
                    ...
                    reject(new Error(`Failed to get file data for fileId ${fileId} after ${attempts} attempts.`));
                } else {
                    attempts++;
    ...
    useEffect(() => {
        ...
        const initSDK = async () => {
            try {
                        ...
                        if (isTutor) {
                            ...
                            if (durationInMs) {
                                ...
                                bookingEndTimerRef.current = setTimeout(() => {
                                    if (isMountedRef.current) {
                                        alert('The scheduled class time has ended. Please end the class.');
                                    }
                                }, durationInMs);
                            }
                        }
                ...
                ConnectyCube.videochat.onCallListener = async (callSession, extension) => {
                    ...
                    if (!isTutor) {
                        ...
                        try {
                            ...
                            if (isMountedRef.current) {
                                setError('Failed to join call. Please check microphone and camera permissions.');
                                setCallState('ended');
                            }
                            callSession.reject({});
                        }
                ...
                ConnectyCube.videochat.onUserNotAnswerListener = (callSession, userId) => {
                ...
                ConnectyCube.videochat.onRejectCallListener = (callSession, userId, extension) => {
                ...
                ConnectyCube.videochat.onStopCallListener = (callSession, userId, extension) => {
                ...
                if (isMountedRef.current) {
                    setError(`Failed to connect: ${authError.message || 'Please check credentials and try again.'}`);
                    setCallState('ended');
                }
            }
        };
        ...
        return () => {
            ...
            if (originalListeners.onRemoteStreamListener !== null) {
                ConnectyCube.videochat.onRemoteStreamListener = originalListeners.onRemoteStreamListener;
            }
            if (originalListeners.onCallListener !== null) {
                ConnectyCube.videochat.onCallListener = originalListeners.onCallListener;
            }
            if (originalListeners.onUserNotAnswerListener !== null) {
                ConnectyCube.videochat.onUserNotAnswerListener = originalListeners.onUserNotAnswerListener;
            }
            if (originalListeners.onRejectCallListener !== null) {
                ConnectyCube.videochat.onRejectCallListener = originalListeners.onRejectCallListener;
            }
            if (originalListeners.onStopCallListener !== null) {
                ConnectyCube.videochat.onStopCallListener = originalListeners.onStopCallListener;
            }
            if (originalListeners.onDisconnectedListener !== null) {
                ConnectyCube.chat.onDisconnectedListener = originalListeners.onDisconnectedListener;
            }
    ...
    const startCall = async () => {
        ...
        try {
            ...
            if (!hasPermissions) {
                throw new Error('Media permissions denied. Please allow camera and microphone access.');
            }
            if (!isMountedRef.current) return;
            if (!ConnectyCube.chat.isConnected) {
        ...
        return (
            <div className="waiting-screen">
                <div className="waiting-content">
                    ...
                    <p>{booking.subject} - {new Date(booking.startDate).toLocaleDateString()} {booking.startTime}</p>
    ...
    const sendUpdatesWithDebounce = (elements) => {
        ...
        debounceHandlerRef.current = setTimeout(() => {
            ...
            if (!isMountedRef.current) {
                console.error('SENDER (Firestore): Cannot send element update, not ready.');
                return;
            }
    ...
    const handleWhiteboardChange = (elements, appState, files) => {
        ...
        const oldIds = lastSentElementsRef.current ? new Set(lastSentElementsRef.current.map(el => `${el.id}-${el.version}`)) : new Set();
        ...
        if (hasChanged) {
            console.warn('SENDER: Detected element changes, queueing Firestore update.');
        ...
        for (const element of elements) {
            ...
            if (element.type === 'image' && element.fileId && !sentFilesRef.current.has(element.fileId)) {

                const newFileId = element.fileId;
                sentFilesRef.current.add(newFileId); // Mark as "sending"

                console.log(`SENDER (Firestore): Detected new image (fileId: ${newFileId}). Polling...`);
                ...
                waitForFileData(newFileId)
                    .then(fileData => {
                        ...
                        // --- Use Firestore service ---
                        console.log(`SENDER (Firestore): Sending file data for ${newFileId}.`);
                        addWhiteboardFile(booking._id, fileData, currentUser.cb_id);
                    })
                    .catch(err => {
                        if (!isMountedRef.current) return;
                        console.error(`SENDER (Firestore): Poller for ${newFileId} failed.`, err.message);
    ...
    return (
        <div className="video-call-page">
            ...
            <footer className="call-footer">
                ...
                <p>{new Date(booking.startDate).toLocaleDateString()} | {booking.startTime} - {booking.endTime}</p>
