...
export function TutorDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      setShowCityWarning(
        user.role === "tutor" &&
          !user.city.toLowerCase().includes("bengalore") &&
          !user.tutoring_mode.includes("Online Tutoring"),
      );
    } catch (error) {
      console.error(error);
    }
    ...
    if (user && user.verificationStatus === "verified") {
      const fetchData = async () => {
        try {
          ...
          const [insightsData, newRequestsData, activeBookingsData] =
            await Promise.all([
              insightsDataPromise,
              newRequestsPromise,
          ...
          if (activeBookingsData && activeBookingsData.bookings) {
            ...
            const todayClasses = activeBookingsData.bookings.filter(
              (booking) => {
                const bookingStartDate = new Date(booking.startDate).setHours(
                  0,
                  0,
                  0,
                  0,
      ...
      <main className="dashboard-main">
        <div className="topbar">
          ...
          <div className="profile">
            ...
            <div className="welcome-text">
              Welcome back,{" "}
              {user.fullName ? user.fullName.split(" ")[0] : "Unknown"}
            </div>
          </div>
        </div>
        ...
        {showCityWarning && (
          <div className="review-card">
            <h2>
              Home tutoring services are currently available exclusively in
              Bengaluru.
            </h2>
            <p>
              You may also select the online tutoring option in your{" "}
              <a
                href="/tutor-account"
                style={{
                  textDecoration: "none",
              ...
              to connect with students from anywhere.
            </p>
          </div>
        )}
        {isUnderReview ? (
            ...
            <p>
              {user.verificationStatus === "rejected"
                ? user.rejectionReason ||
                  "Resubmit your details to request again."
                : "We’re currently reviewing your account details. Once approved, you’ll have full access to your dashboard features."}
            </p>
            {user.verificationStatus === "rejected" && (
              <button
                className="primary-btn"
                onClick={() => navigate("/verify-tutor")}
              >
