...
const VideoCall = () => {
  ...
  const applyMediaSettings = (session, stream) => {
    if (!stream) return;
    stream.getAudioTracks().forEach((track) => { track.enabled = !isMutedAudio; });
    stream.getVideoTracks().forEach((track) => { track.enabled = !isMutedVideo; });
  ...
  useEffect(() => {
    ...
    const initSDK = async () => {
      try {
        ...
        // FIX 1: Use remoteVideoRef.current instead of "remoteVideo" string
  ...
  const joinCall = async () => {
    try {
      ...
      // FIX 3: Use localVideoRef.current instead of string
      session.attachMediaStream(localVideoRef.current, stream, { muted: true });

      session.accept({});
      setCallState("active");
    } catch (err) { setError("Media error"); }
  };
  ...
  return (
    <div className="video-call-page">
      ...
      <div className="video-container">
        {/* IMPORTANT: Use the Refs for these elements */}
