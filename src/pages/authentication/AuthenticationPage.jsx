...
export default function AuthenticationPage() {
  ...
  const infoBulletPoints = {
    true: [
      // student
      {
        text: "Register and create your learning profile",
        icon: <FaUserGraduate className="infoIcon" />,
      },
      {
        text: "Choose the subject or skill you want",
        icon: <FaListUl className="infoIcon" />,
      },
      {
        text: "Get matched with the right tutor",
        icon: <FaSearch className="infoIcon" />,
      },
      {
        text: "Learn online using our interactive board and receive downloadable notes for easy revision",
        icon: <FaChalkboardTeacher className="infoIcon" />,
      },
    ],
    false: [
      // tutor
      {
        ...
        icon: <FaUserTie className="infoIcon" />,
      },
      {
        text: "We verify your expertise",
        icon: <FaCertificate className="infoIcon" />,
      },
      {
        text: "Get matched with students or projects",
        icon: <FaUsers className="infoIcon" />,
      },
      {
        text: "Teach / deliver work",
        icon: <FaCheckCircle className="infoIcon" />,
      },
      {
        text: "Receive timely payments",
        icon: <FaMoneyBillWave className="infoIcon" />,
      },
    ],
  };
  ...
  useEffect(() => {
    ...
    setFormTitle(
      userRole === "student"
        ? `${prefix} Your Student Account`
        : `${prefix} Your Tutor Account`,
    );
    setIsStudentOfOnlineTutoring(
      userRole === "student" && tutoring_mode === "Online Tutoring",
    );
  ...
  useEffect(() => {
    ...
    if (step === 1) {
      if (isLoggingInRef.current) {
        ...
        setFormTitle(
          isStudent
            ? "Join Xplooreze — Learn Smarter, Grow Faster"
            : "Become a Tutor with Xplooreze",
        );
      }
    } else if (step === 2) {
  ...
  const fetchSubCategories = async (category) => {
    ...
    if (
      !data.some((subcat) =>
        (formData.subcategory || []).includes(subCategories),
      )
    ) {
  ...
  const handleGoogleSignIn = async (response) => {
    if (response?.credential) {
      try {
        ...
        if (
          (result.role === "student" || result.role === "tutor") &&
          result.isFormSubmitted === true
        ) {
          if (
            isLoggingInRef.current ||
            (role.current === "student" && result.role === "student") ||
            (role.current === "tutor" && result.role === "tutor")
          ) {
            ...
            showErrorDialog(
              "Account Exists",
              `You already have an account as ${result.role}.`,
            );
          }

          // Block 2: New user or user with incomplete form
        } else {
          if (isLoggingInRef.current === false) {
            // === "is registering"
            ...
            if (result.role === "student") {
              ...
              showErrorDialog(
                "Account Not Found",
                "No account exists with this email. Create one to proceed.",
              );
            }
          }
        }
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (formData.avl_days.length === 0 || formData.avl_time.length === 0) {
      showErrorDialog(
        "Error",
        "Please select your availability (from Step 5).",
      );
      return;
    }
  ...
  const renderStepContent = () => {
            ...
            <div className="form-group">
              ...
              <MultiSelect
                ...
                options={[
                  ...
                  "Adults (Hobby Learners)",
                ]}
                value={formData.age_group || []}
                onChange={(items) =>
                  setFormData((prev) => ({ ...prev, age_group: items }))
                }
              />
            </div>
          )}
      ...
      return (
        <>
          ...
          <div className="form-group">
            ...
            >
              Please ensure that the certificates uploaded correspond to the
              subject(s) you wish to teach.
            </small>
          </div>
          ...
          <div className="form-group">
            ...
            >
              Upload a government ID with address proof (like Aadhaar, Voter ID,
              or Driving Licence).
            </small>
          </div>
        </>
    ...
    if (step === 5) {
      ...
      const tutoringModes = (formData.tutoring_mode || []).map((mode) =>
        mode.toLowerCase(),
      );
      ...
      const needsPincodes =
        tutoringModes.length > 0 &&
        tutoringModes.some((mode) => mode !== "online tutoring");
      // --- END UPDATED LOGIC ---
    ...
    if (step === 6) {
      ...
      return (
        <>
          ...
          >
            Rate will be updated automatically based on the duration selected by
            the student.
          </small>
        </>
      );
  ...
  return (
    <div className="authentication-root">
      {(step > 1 || isLoggingInRef.current) && (
        <div className="side-bar">
          {step > 1 && (
            <FaArrowLeft className="back-icon" onClick={handlePreviousStep} />
          )}
          {isStudentOfOnlineTutoring != null && (
            <video
              autoPlay
              muted
              // loop
            ...
            >
              <source
                src={
                  isStudentOfOnlineTutoring
                    ? "/online-tutoring-animation.mp4"
                    : isStudent
                      ? "/login-animation.mp4"
                      : "/tutor-login-animation.mp4"
                }
                type="video/mp4"
              />
      ...
      <div className="authentication-container">
        <div className="authentication-box">
          ...
          {isStudentOfOnlineTutoring != null &&
            ...
            !isLoggingInRef.current && (
              <>
                ...
                >
                  {isStudent
                    ? "Find tutors for everything — from school academics to college subjects, competitive exams, languages, and real-world skills."
                    : "Share your expertise. Earn from anywhere. Teach on your schedule."}
                </p>
                <video
                  autoPlay
                  muted
          ...
          <form
            className="form"
            onSubmit={isFinalStep ? handleFormSubmit : handleNextStep}
          >
            {renderStepContent()}
      ...
      {step < 2 && !isLoggingInRef.current && (
        <div className="registration-info">
          <h2>{"Who Can Join"}</h2>
          <p>
            {isStudent
              ? "Students and professionals looking for personalised and flexible learning system."
              : "Anyone skilled in a subject — school, college, competitive exams, skills, languages, or academic writing/research."}
          </p>
          <h2>{"How It Works"}</h2>
          <div className="info-points-root">
            {infoBulletPoints[isStudent].map((data) => (
              <p key={data.text}>
                {data.icon} {data.text}
              </p>
            ))}
