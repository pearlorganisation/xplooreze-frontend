...
import { getTutors, checkTrialAvailability, getUpcomingClasses } from "../../../data/modules/student-module";
...
export default function StudentDashboard() {
    ...
    const timeOptions = ['0-3', '3-6', '6-9', '9-12', '12-15', '15-18', '18-21', '21-24'];
    ...
    const fetchTutors = useCallback(async () => {
        setIsLoading(true);
        try {
            // We can just pass searchParams directly, api.get will serialize it
            const params = new URLSearchParams(searchParams);

            // Ensure default limit/page if not present
            if (!params.has('page')) params.set('page', '1');
            if (!params.has('limit')) params.set('limit', '10');
            if (!params.has('sortBy')) params.set('sortBy', 'relevance');

            // Pass the searchParams object itself
    ...
    }, [searchParams]); // Re-run *only* when searchParams changes
    ...
    useEffect(() => {
        ...
        async function fetchStaticData() {
            try {
                ...
                setCategories(cats);
                const subsPromises = cats.map(cat => getSubCategories({ category: cat }));
        ...
        async function setupClassReminders() {
            try {
                ...
                classes.forEach(classInfo => {
                    ...
                    const fiveMinutesBefore = new Date(classStart.getTime() - 5 * 60 * 1000);
                    ...
                    if (timeUntilShow <= 0 && timeUntilHide > 0) {
                        setActiveReminders(prev => {
                            // Avoid duplicates
                            if (prev.some(r => r.bookingId === classInfo.bookingId)) return prev;
    ...
    const handleFilterChange = (key, value) => {
        ...
        } else { // Handle string/number values
            if (value) {
    ...
    return (
        <div className="student-dashboard">
            ...
            <div className={`filters-section ${!showFilters && isMobile ? 'hidden' : ''}`}>
            ...
            <div className='tutors-list'>
                {tutors.length === 0 ? (
                    <p className="no-results">
                        {totalResults === 0 && !isLoading ? "No tutors found matching your criteria." : ""}
                    </p>
                ) : (
                    tutors.map(tutor => (
                        <div key={tutor._id}>
